/**
 * Daily streak E2E suite — Phase 8 / TASK-009.
 *
...
test.describe("Daily streak — Check in", () => {
  ...
  test("Streak persists across page reload (localStorage round-trip)", async ({
    page,
    consoleErrors,
  }) => {
    await page.goto("/daily");
    ...
    // Wait for the button to flip to "Checked in" — this is a proxy
    // for the streak state having flushed (localStorage write is sync
    // inside handleCheckIn but we still wait for the React re-render).
    await expect(
      page.getByRole("button", { name: /already checked in today/i }),
    ).toBeDisabled({ timeout: 5_000 });

    // Hard reload. The qk-demo-daily-streak key should hydrate the
    // counter back to 1 BEFORE any network call (the localStorage read
    // runs synchronously in the initial useState factory).
    await page.reload();
    ...
    // After reload, the localStorage-backed streak hydrates BEFORE any
    // network call (initial useState factory in daily.tsx reads
    // qk-demo-daily-streak synchronously). The strongest signal is:
    //   (a) the "Already checked in today" button stays disabled
    //       (claimedToday derived from streak.lastTimestamp), AND
