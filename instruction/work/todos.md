...
- **Subtasks:**
  ...
  - [x] verify (Lead): root gates GREEN (14/14 typecheck, 10/10 lint; react 152/0 +7 from F4-c, worker-api 216/0/1-skip +5 from F4-a +2 from F4-b)
  - [x] bump (Lead): 0.1.11 → 0.1.12 in package.json + /v1/health + CHANGELOG prepended with full F4 batch entry
  - [x] commit (Lead): `aa8df37 v0.1.12 hotfix — F4 batch (SpinWheel + completed dedup + Deep Diver)` — no AI signature
  - [x] push + monitor (Lead): CI `26266044014` GREEN 3m30s; Deploy `26266151648` workers + smoke + D1 migrations (incl. 0005) GREEN, E2E red (TASK-005 manual gate as expected)
  - [x] prod verify (Lead): `/v1/health` → `{"ok":true,"version":"0.1.12"}`; Playwright with `?user=v012_verify` (fresh): F4-a Deep Diver 0→6/10 after 6 long-video clicks ✅; F4-b Daily Watcher `mission.completed` fires **1× only** (not 6× as in v0.1.11 — confirmed via console.debug: `before: undefined, after: 1` then no re-fires) ✅; F4-c SpinWheel `rotate(2070deg)` (=270°) for announced "Streak +1!" — slice geometrically at pointer = slice 1 "Streak +1!" (MATCHES announcement) ✅. Screenshot: `agent-temp/v0.1.12-verify-spinwheel-sync.png`. **0 errors, 0 warnings in console**
  - [ ] archive (Lead): `/workflow-end` after user confirmation
- **Progress Notes:**
  - 2026-05-22 09:35 - TASK-013 created. 3 sub-agents dispatching in parallel.
  - 2026-05-22 10:20 - Lead release pipeline complete. Root gates GREEN (14/14 typecheck, 10/10 lint, 500+ tests). Version bumped 0.1.11→0.1.12 in package.json + /v1/health route + CHANGELOG prepended with full F4 batch entry. Committed `aa8df37`. Pushed. CI GREEN in 3m30s. Deploy: workers + smoke GREEN incl. D1 migration 0005 applied cleanly, E2E red (expected — TASK-005 manual gate). `/v1/health` = 0.1.12 confirmed.
  - 2026-05-22 10:25 - Playwright F4 re-verify on prod with `?user=v012_verify` (fresh user). Hard evidence per fix: **F4-a** Deep Diver 0/10 → **6/10** after clicking all 6 videos (all ≥ 20min) — was 0/10 in v0.1.11 verify; **F4-b** Daily Watcher `mission.completed` SSE event fired **1 time only** (`before: undefined, after: 1`) instead of 6× as in v0.1.11 — terminal→terminal skip working; **F4-c** SpinWheel announced "Streak +1!" (slice 1) → final rotation `2070deg` = 270° → slice geometrically at pointer = slice 1 ("Streak +1!") — VISUAL MATCHES ANNOUNCEMENT (v0.1.11 same prize was 1980° → slice 4 "Bonus tick!" wrong). Console: 0 errors, 0 warnings, 18 debug logs showing accurate before/after counts. Screenshots: `agent-temp/v0.1.12-verify-spinwheel-sync.png`. F1+F2+F3+F4-a/b/c all closed end-to-end.
...
### Task: [TASK-014] v0.1.13 batch — F5-a/b/c (UX gaps surfaced by user re-test of v0.1.12)

- **Status:** 🔵 in-progress (sub-agent A complete; B+C in flight)
- **Priority:** P1
- **Parallel:** yes (3 independent file scopes — minigames.tsx / Layout.tsx / streaming.tsx)
- **Assigned:** Lead (Opus 4.7) + sub-agents A, B, C (all Opus 4.7)
- **Depends on:** TASK-013 (v0.1.12 shipped)
- **Covers:** F5-a (minigame mission card placement) + F5-b (balance multi-currency) + F5-c ("Watched today" label)
- **Sub-agent assignments:**
  - **A (F5-a, minigame mission card placer)**: `apps/demo/src/routes/minigames.tsx` — render `mis_lucky_spinner` + `mis_scratch_master` MissionCards in-place so users can claim without leaving /minigames.
  - **B (F5-b)**: `apps/demo/src/components/Layout.tsx` — balance multi-currency display.
  - **C (F5-c)**: `apps/demo/src/routes/streaming.tsx` — "Watched today" label fix.
- **Subtasks:**
  - [x] A: add minigame mission cards
  - [x] B: balance multi-currency
  - [x] C: watched today re-label
  - [ ] verify (Lead): root gates GREEN
  - [ ] bump + commit + push + prod verify
- **Progress Notes:**
  - 2026-05-22 11:15 - Sub-agent A complete. New `Mini-game missions` region added BETWEEN the Spin/Scratch widget grid and the "How the mini-games connect" info block. Wired: `useMissions()` (no campaignId — minigame missions live in the global campaign) filtered client-side via `MINIGAME_MISSION_IDS = ["mis_lucky_spinner","mis_scratch_master"]`; iterates MissionCard manually (mirrors /streaming TASK-002 pattern rather than using `<MissionList />` so the same in-place style/empty-state UX applies); claim handler wires to `useMissionClaim({ onClaimed: missionsState.refetch })` for the SSE-degraded backstop established in v0.1.9. Docblock extended to explain F5-a / v0.1.13 rationale. Demo gates: test 8/8 in 3 suites GREEN, typecheck GREEN, lint GREEN for minigames.tsx (B's Layout.tsx + Layout.test.tsx lint errors are out of A's scope). E2E coverage deferred to Lead's Playwright run.
